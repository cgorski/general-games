...
-- evalTree :: (Ord v, Eq v, Num v, Fractional v) => Tree Operation v -> v
-- evalTree (Leaf v) = v
...
-- evalTree (Node Divide l r) =
--   let el = evalTree l
--       er = evalTree r in
--     if el == 0 || er == 0
--     then 0
--     else el / er
-- evalTree (Node LAttenuatedByR l r) =
--   let el = evalTree l
--       er = evalTree r in
--     if el > er then er else 0
-- evalTree (Node RAttenuatedByL l r) =
--   let el = evalTree l
--       er = evalTree r in
--     if er > el then el else 0
...
--     randOp = do 
--       rNum :: Int <- getRandomR(0,fromEnum (maxBound :: Operation))
--       return $ toEnum rNum
